...
export function renderPage(route: string, base: string) {
  ...
  switch (route) {
    case "":
      content = `<section class="hero"><div class="hero-image">${img("bto-inspection-egates", "", true)}</div><div class="hero-shade"></div><div class="hero-copy"><p class="eyebrow">Olubunmi Tunji-Ojo · Nigeria</p><h1>TUNJI<span>OJO.</span></h1><div class="hero-bottom"><p>A life in service.<br>A record in focus.</p><a class="circle-link" href="#introduction" aria-label="Explore the story">↓</a></div></div><span class="hero-caption">In the field / Lagos, 2024</span></section>
 ...
 <section class="section wrap letter-teaser"><p class="eyebrow">A student constituency’s perspective</p><h2 class="display reveal">A RECORD<br>THAT SPEAKS.</h2><div><p class="lede">A vote of confidence from NANS Southwest Zone D.</p><p>The letter that occasioned this documented record: an assessment of public service and its impact on students.</p>${arrow("Read the full letter", url("letter"))}</div></section>`;
      ...
      content =
        ...
        `<section class="wrap reading bottom-space faq">${[
          ...
          [
            ...
            "Photographs include the minister’s official website and credited public reporting. Films are embedded from Channels Television only when you choose to play them.",
            "media",
    ...
    case "privacy":
      content =
        ...
        `<section class="wrap reading bottom-space"><h2>No accounts. No forms.</h2><p>This site does not ask you to create an account or submit personal details. It does not include advertising or analytics scripts, and it does not set application cookies.</p><h2>Films load when you choose.</h2><p>YouTube videos use the privacy-enhanced youtube-nocookie.com player. A connection to YouTube is made only when you open a film. Playback is then subject to YouTube’s privacy practices.</p><h2>Fonts and photographs</h2><p>Fonts and displayed photographs are served with this website. Opening a source link takes you to a separate publisher with its own privacy policy.</p><h2>Hosting</h2><p>The published site uses GitHub Pages. The hosting provider may process technical information such as IP addresses to deliver and secure the site.</p>${arrow("GitHub privacy statement", "https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement")}<h2>Sharing</h2><p>Copy buttons write the selected text to your clipboard when you choose to use them. Nothing is posted or sent automatically.</p></section>`;
      break;
    case "share":
      content =
        head(
